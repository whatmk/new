 ...
 */
export function create(post, name){
	return post("/v1/role/create", {name:name})
}
 ...
 * @return {[type]}      [description]
 */
export function update(post, id, name, ts){
	return post("v1/role/update", {id:id, name:name,  ts:ts})
}
...
export function checkMenu(post, menuId){
    if (sessionStorage.getItem('dzOrgId')) {
        return post("v1/role/checkMenu", {menuId,sign:1})
    }
	return post("v1/role/checkMenu", {menuId})
}
